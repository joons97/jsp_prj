...
public class BoardUtil {
	...
	public String pagination(SearchVO sVO) {
		...
		if(sVO.getTotalCount() != 0) {
			...
			if( sVO.getCurrentPage() > pageNumber ){//현재 페이지가 pagination보다 크다면
			...
			while( movePage <= endPage ){
				...
				if(movePage == sVO.getCurrentPage()){//현재 페이지에서는 링크를 활성화 할 필요가 없다.
					pageLink.append("[").append(movePage)
					.append("]");
				}else {//현재 페이지가 아니면 링크를 설정한다.
					pageLink
					.append("[<a href='")
					.append(sVO.getUrl())
					.append("?currentPage=")
